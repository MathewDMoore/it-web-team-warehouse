...
class ScanController {
	...
	ClearDelivery(docNumber) {
		...
		modalInstance.result.then(() => {
			this.ScanOrderService.ClearDelivery({ DeliveryNumber: docNumber, IsInternal: this.Delivery.IsInternal }).then((result) => {
				if(result.data) {
					this.FirebaseDeliveryService.Delete(this.Delivery).then(() => {
	...
	ExportCSV() {
		...
		_.each(data, (item) => {
			...
			_.each(values, (text, columnIndex) => {
				if(_.isString(text)) {
	...
	IsScanComplete() {
		...
		var notScannedValid = (this.Delivery.NotScannedItems && this.Delivery.NotScannedItems.length > 0) ? this.Delivery.NotScannedItems.filter((i) => { return (!i.ProductId || i.SmartCodeOnly && i.NoSerialRequired || (i.SmartCodeOnly && !i.NoSerialRequired)) }).length === 0 : true;
