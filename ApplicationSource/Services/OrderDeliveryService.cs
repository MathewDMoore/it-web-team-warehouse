	...
	public class OrderDeliveryService: IOrderDeliveryService
	{
		...
		public OrderDeliveryModel OrderLookUp(MacDeliveryModel lookup)
		{
			...
			if(delivery != null)
			{
				...
				var items = _repo.GetDeliveryItems(new DeliveryOrderItemsQuery
				{
		...
		public SaveScanModel MatchAndSave(MatchModel scanModel)
		{
			...
			if(IsUserTimedOut())
			{
				if(!string.IsNullOrEmpty(scanModel.SerialCode) && scanModel.SerialCode.Length >= 7)
				{
					...
					var matches = _repo.FindUnScannedMatch(new SerialNumberItemQuery
					{
						ProductId = productId,
						Color = color,
					...
					if(matches.Any())
					{
						...
						if(scanModel.KitId > 0)
						{
							...
							if(match != null)
							{
								...
								model.ErrorMessage = "No matches found in kit. You may have scanned the bar code incorrectly";
						}
						else
						{
							var list = matches.All(m => m.KitId == 0) || matches.All(m => m.KitId > 0) ? matches : matches.Where(m => m.KitId.Equals(0)).ToList();

							while(matchIndex <= list.Count - 1)
							{
								...
								if(string.IsNullOrEmpty(error))
								{
									var updated = _repo.UpdateSerialNumberItem(item, scanModel.IsInternal);
		...
		private void FindActiveKits(ICollection<SerialNumberItem> items, OrderDeliveryModel deliveryModel)
		{
